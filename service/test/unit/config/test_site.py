...
class TestPixelatedSite(unittest.TestCase):
    ...
    def test_add_security_headers(self):
        ...
        self.assertEquals(header_value, request.responseHeaders.getRawHeaders(
            'X-Content-Security-Policy'.lower())[0])
        self.assertEquals(header_value, request.responseHeaders.getRawHeaders(
            'Content-Security-Policy'.lower())[0])
        self.assertEquals(header_value, request.responseHeaders.getRawHeaders(
            'X-Webkit-CSP'.lower())[0])
    ...
    def test_does_not_add_strict_transport_security_header_if_plain_http(self):
        ...
        self.assertFalse(request.responseHeaders.hasHeader(
            'Strict-Transport-Security'.lower()))
